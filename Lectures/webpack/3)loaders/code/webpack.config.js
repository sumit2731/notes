...
module.exports = {
    ...
    output: {
        /* 
        here contenthash means add md5 hash to name of file. it will chnage only if content of file cahnges.
        if content remains same , md5 hash remains same. this helps in browser caching
        */
        ...
        *Public path tells webpack where generated files are located, if we dnt give public path, then it
        will look at 'path' where output files are generated. when webpack creates path to the file, it will 
    ...
    module: {
        ...
        */
        rules: [
            ...
            syle-loader bundles your css togather with js into a single resultant file.At runtime this css is aded to our 
            html in style tag. in future videos we will see how to generte css in separate file.
            */
           {
                test: /\.css$/,
                use: [
    ...
    plugins: [
        ...
        new CleanWebpackPlugin({
            cleanOnceBeforeBuildPatterns: [
                /* 
                All relative path are relative to output.path folder. if we dnt specify cleanOnceBeforeBuildPatterns then by defualt
                it clears all folder in outout.path property. this is what we mean in this pattren
                */
                '**/*',
                /* 
                *If we want to clean any folder outside output.path then we need to use asolute path. here we want to clear build folder,
                all files as well as folders
                */
                path.join(process.cwd(),'build/**/*')
            ]
        }),
        /* 
        This generates a simple html file. which adds imports of all js and css
