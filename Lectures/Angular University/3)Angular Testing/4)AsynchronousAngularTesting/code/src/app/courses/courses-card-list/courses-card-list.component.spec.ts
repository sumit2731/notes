...
describe('CoursesCardListComponent', () => {
  ...
  *In order to create a component instance we need a component fixture
  component fixture is test utility type that is going to help us to do
  ...
  *Here we are uisng Angular testing utility async because we
  ...
  this utility is designed specifically to solve this problem. to async we pass
  callback that we previously passed to beforeEach. this is how async works, it waits
  for any aync operations triggered by function passed to it, to complete. so async is going to
  wait for predefined amount of time, by defualt that is timeout of 5 seconds for any asynchrnous
  ...
  by this code, and async is then by defualt going to wait for 5 seconds for all those operations to
  complete, before cosnidering beforeEach step has been completed

  here only asynechrnous operation is promise, so here async is going to detect the promise
  ...
  */
  beforeEach(async(() => {
    ...
    *Any services it needs will be provided via poviders, here we 
    ...
    course module.this will bring not only CourseCardListComponent but also all needed angular
