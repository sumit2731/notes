...
module.exports = {
  ...
  /**
   * This option is avalaible from version -webpack4. prior to that much more configurations were required to get same results.
   * None- means we do not want any built in optimizations
   * productions -
   *  a)sets process.env.NODE_ENV === 'production'
   *  b)enables a list of plugins including terser plugin. full list - https://webpack.js.org/configuration/mode/
   *  c)sourcemaps are not shipped , so when we get error,from console we get to bundle file. in dev mode
   *    source maps are there so we get to specific line in concerned file
   */
  // mode: "none",
  mode: "production",
  // mode: "development"
