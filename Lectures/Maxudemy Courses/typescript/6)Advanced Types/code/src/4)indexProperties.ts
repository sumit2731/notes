interface ErrorContainer4 {
  /*
    ...
    With that we still can also add predefined properties,however only if they are of same type as
    this.so we add id proeperty. so object will now have id property and it 
    can add as many other properties as it wants. type of id needs to be string only, as we
    have index type property which is of type string, so it is a restriction
    */
  [prop: string]: string;
  /**
   * @Desc all properties needs to be string
   */
  //id: number;
  //id: string;
}
 ...
 *
 * Ts Docs -
 *  https://www.typescriptlang.org/docs/handbook/2/objects.html#index-signatures (index signature)
 *  https://www.typescriptlang.org/docs/handbook/2/keyof-types.htm (using index signature and 'in' and 'keyof')
 * https://www.typescriptlang.org/docs/handbook/2/mapped-types.html (mapped types)
 *  l (also read next read 3-4 articles )
 */
...
/**
  ...
  see link -https://www.typescriptlang.org/docs/handbook/2/keyof-types.html (read next 3 or 4 articles)
  for code ee file 8)
  */
