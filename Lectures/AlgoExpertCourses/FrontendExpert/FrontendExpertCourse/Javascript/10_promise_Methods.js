...
Promise.myAny = function(promiseArray) {
    return new Promise((resolve, reject) => {
        ...
        for(let promise of promiseArray) {
            ...
            .catch(err => {
                failedPromiseCount++;
                if(failedPromiseCount == promiseArray.length) reject("all promises rejected");
            })
        }
    });
}
 ...
 */
Promise.myAllSettled = function (promiseArray) {
    let newPromiseArray = promiseArray.map(promise => (promise.then(value => ({status: 'fulfilled', value}))))
